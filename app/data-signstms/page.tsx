...
export default function DataSIGNSTMSPage() {
  const { username } = useAuth();
  const [data, setData] = useState<SIGNSTMSData[]>([]);
  ...
  const fetchData = async (page: number = 1) => {
    try {
      ...
      const response = await fetch(`/api/data-signstms?page=${page}&limit=${pagination.limit}${hprocParam}`);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-4">ข้อมูล SIGNSTMS</h1>
          <div className="flex gap-4">
            <Link 
              href="/upload-signstms"
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">MType</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {data.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-2 py-4 whitespace-nowrap text-sm text-gray-900">{item.mtype}</td>
