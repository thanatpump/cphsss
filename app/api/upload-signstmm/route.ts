...
export async function POST(request: NextRequest) {
  try {
    ...
    // ตรวจสอบว่าเป็นไฟล์ SIGNSTMM หรือไม่
    if (!file.name.includes('SIGNSTMM')) {
      return NextResponse.json({ error: 'ไฟล์นี้ไม่ใช่ไฟล์ SIGNSTMM กรุณาเลือกไฟล์ที่ถูกต้อง' }, { status: 400 });
    }
    ...
    // ตรวจสอบซ้ำ
    const [existRows]: any[] = await db.query('SELECT id FROM signstmm WHERE stmno = ?', [STMdoc]);
    ...
    for (const bill of billsArray) {
      if (bill) {
        await db.query(`
          INSERT INTO signstmm (
            period, stmno, dateDue, hmain, hcode, hproc, hn, an, pid, name,
            dateadm, datedsc, ft, bf, drg, rw, adjrw, due, ptype, rwtype, rptype,
    ...
    return NextResponse.json({ 
      success: true, 
      message: `อัปโหลดข้อมูล SIGNSTMM สำเร็จ ${insertedCount} รายการ`,
      count: insertedCount
    });
    
  } catch (error) {
    console.error('Error uploading SIGNSTMM:', error);
    return NextResponse.json({ 
