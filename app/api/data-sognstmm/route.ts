...
export async function GET(request: NextRequest) {
  try {
    ...
    const countQuery = hproc ? 
      'SELECT COUNT(*) as total FROM sognstmm s WHERE s.hproc = ?' : 
      'SELECT COUNT(*) as total FROM sognstmm';
    ...
    const [data] = await db.query(`
      SELECT 
        ...
        s.hmain, s.hproc, s.hcare, s.hn, s.pid, s.name, s.bf, s.pcode, s.care,
        s.payplan, s.bp, s.invno, s.dttran, s.copay, s.cfh, s.total, s.rid, s.created_at,
        h.name as hosp_name
      FROM sognstmm s
    ...
    console.error('Error fetching SOGNSTMM data:', error);
    return NextResponse.json({ 
